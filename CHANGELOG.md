...
* **📦 Leaner app bundle & accurate deployment docs** — development files (README, driver guides, `build_and_notarize.sh`) no longer ship inside `Contents/Resources`; the sample mobileconfig and README now document that macOS's Local Network prompt has no MDM payload (users must Allow once) and include the `com.apple.servicemanagement` payload that pre-approves the privileged helper fleet-wide.
...
* **🪪 AirPrint installs no longer rejected by CUPS** — the advertised model used as the printer name contains spaces, which `lpadmin` refuses ("Printer name can only contain printable characters"), so every AirPrint install failed. IPP and Bonjour-fallback names now pass through the same CUPS-safe sanitizer the SMB path has always used (spaces and punctuation become hyphens).
* **🖨️ AirPrint / IPP Everywhere verification works** — network scans now capture Bonjour TXT records (`ippfind -x` template) and detection uses the real AirPrint signature (a `URF` TXT record / `image/urf` in `pdl`) instead of nonexistent `airprint=true` keys, so AirPrint printers are badged and preferred correctly; the advertised model (`ty`) is used as the display name. The `ipp://` and `ipps://` adverts of one physical printer are collapsed into a single entry (matched by UUID TXT record, else host + name), preferring the secure `ipps` variant.
* **📡 mDNS discovery actually returns printers** — `ippfind` was invoked with `-q` (quiet mode), which suppresses all output, so even a successful scan parsed as zero printers; the flag is removed. Additionally, IPP-family `dnssd://` entries from cupsd's own Bonjour browse (`lpinfo -v`) are now parsed and used as a discovery fallback when the app-level scans return nothing — cupsd browses with its own privileges, so printers still appear even if macOS denies the app's Local Network access.
